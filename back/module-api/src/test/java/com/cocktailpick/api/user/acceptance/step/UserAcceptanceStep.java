...
import com.cocktailpick.api.user.dto.UserResponse;
...
public class UserAcceptanceStep {
	public static ExtractableResponse<Response> requestToGetCurrentUser(AuthResponse authResponse) {
		return given().log().all()
			.header(HttpHeaders.AUTHORIZATION, authResponse.getTokenType() + " " + authResponse.getAccessToken())
			.when()
			.get("/api/user/me")
			.then().log().all()
			.extract();
	...
	public static ExtractableResponse<Response> requestToAddFavorite(AuthResponse authResponse,
		...
		return given().log().all()
			.header(HttpHeaders.AUTHORIZATION, String.format("%s %s", authResponse.getTokenType(), authResponse.getAccessToken()))
			.contentType(MediaType.APPLICATION_JSON_VALUE)
