...
import com.cocktailpick.common.domain.DailyDateTest;
...
class NumberOfDailyTest {
	...
	void generateBy(int year, int month, int date, int hour, int minute) {
		Date sixOClock = DailyDateTest.createDate(year, month, date, hour, minute);

		assertThat(NumberOfDaily.generateBy(DailyDate.of(sixOClock)))
			.isEqualTo(NumberOfDaily.generateBy(DailyDate.of(DailyDateTest.createDate(2020, 7, 27, 0, 0))));
	}
}